...
>{: .bash}
>
>カレントディレクトリにあるフォルダーをご覧ください。（訳注：デスクトップ上の`shell-lesson`が見つからない場合は、`explorer.exe .`（起動ファイル名、半角空白、半角ピリオド）と入力し、<kbd>enter</kbd>キーを押す。エクスプローラーが起動するので、その中に`shell-lesson`のフォルダを移動させる）
>4. 最後に入力する：
>
>~~~~
>$ pwd
