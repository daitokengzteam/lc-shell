...
> ## ファイルのコピー
>
> ファイルを **移動**する代わりに、ファイルを **copy**（コピーを作成）したいとき、例えば、ファイルを変更する前にバックアップを作成する場合などです。 mvコマンドと同様に、cpコマンドは古い名前と新しい名前の2つの引数を取ります。`gulliver.txt` という名前のファイルのコピーは、どのように作成しますか？試してみてください。
>
> > ## Answer
> > ~~~
